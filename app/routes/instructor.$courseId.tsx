...
export async function loader({ params, request }: Route.LoaderArgs) {
  ...
  const students = enrolledStudents.map((enrollment) => {
    ...
    const progress = calculateProgress({ userId: enrollment.userId, courseId, includeQuizzes: false, weightByDuration: false });
...
export async function action({ params, request }: Route.ActionArgs) {
  ...
  if (intent === "update-title") {
    updateCourse({ id: courseId, title: parsed.data.title, description: course.description });
    return { success: true, field: "title" };
  }

  if (intent === "update-description") {
    updateCourse({ id: courseId, title: course.title, description: parsed.data.description });
