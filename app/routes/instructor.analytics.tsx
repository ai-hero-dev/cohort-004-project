...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const timeSeries = getRevenueTimeSeries({ instructorId: currentUserId, period });
  const courseBreakdowns = getCourseBreakdowns({ instructorId: currentUserId, period });

  return { summary, timeSeries, courseBreakdowns, period };
}

export default function InstructorAnalytics({ loaderData }: Route.ComponentProps) {
  const { summary, timeSeries, courseBreakdowns, period } = loaderData;
